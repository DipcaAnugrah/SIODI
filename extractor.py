...
def _extract_nomor_kk(text: str) -> str:
    ...
    def _to_16digits(raw: str) -> str:
        """
        Bersihkan raw OCR string → string digit nomor KK.
        ...
        Menangani kasus-kasus noise khas font besar KK:
          ...
          2. Karakter noise di DEPAN nomor: "?2140120212753" — karakter '?'
             ...
             lalu periksa apakah hasilnya digit valid dengan kode provinsi 11-99.
          3. Karakter OCR salah baca: O→0, l→1, I→1, S→5, B→8, dll.

        Panjang digit yang diterima:
          16 digit : standar nasional Dukcapil — langsung dikembalikan
          >16 digit: coba buang 1-2 prefix noise; jika gagal ambil 16 pertama
          15 digit : satu digit terpotong — padding "0" di akhir
          14 digit : nomor non-standar atau translasi "?" menjadi digit tunggal
                     sehingga total 14; dikembalikan apa adanya (TANPA padding)
                     agar tidak menghasilkan angka palsu saat voting
        """
        ...
        # Kasus 14 digit: nomor KK pada dokumen uji (non-standar) atau
        # karakter pertama "?" telah ter-translasi menjadi digit tunggal sehingga
        # panjang akhir 14. Diterima apa adanya — JANGAN dipadding — agar
        # kandidat yang benar dapat masuk voting.
        # Contoh kasus nyata: OCR baca "?3201020245533" → translasi → "73201020245533"
        # (14 digit). Tanpa cabang ini, hasilnya dibuang dan kandidat salah
        # ("7302010202155330" dari kasus 15+padding) memenangkan voting.
        if len(digits) == 14 and re.match(r"[1-9][0-9]", digits[:2]):
            return digits
